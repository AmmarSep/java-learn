...
[Home](../README.md) · [Learning path](../docs/learning-path.md) · [Runnable examples](../examples/README.md)
...
For this repository, run from its root. Each example needs an isolated classpath:

```bash
python3 scripts/java.py list
python3 scripts/java.py run Phase0_HelloWorld
python3 scripts/java.py run Phase0_CommandLineArgs arg1 arg2
python3 scripts/java.py check
```

Generic commands below apply to your own standalone program, not the entire examples tree. See [setup](../docs/setup.md) for a no-Python example that keeps compiled output separate.
...
# Related files in ONE program, not all teaching examples
javac MyClass.java MyHelper.java
java MyClass
...
- Open the repository root and select an installed JDK under **Project Structure**.
- Use the run icon beside `main`, or the shared **Hello Java** configuration.
- Click the gutter to add a breakpoint, then choose **Debug**.
- Use **Search Everywhere** to find a class or file; keyboard shortcuts depend on your selected keymap.
- Keep the per-example modules separate. See [the IntelliJ setup instructions](../docs/setup.md).
